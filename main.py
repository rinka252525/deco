    ...
    for team1_roles in permutations(lanes):
        ...
        for uid, lane in zip(team1_ids, team1_roles):
            ...
            if prefs and lane not in prefs and 'fill' not in prefs:
                success1 = False
                break
            try_role_map1[uid] = lane
        if success1:
            valid_team1 = True
            break

    if not valid_team1:
        continue  # チーム1が希望レーンで組めない場合はスキップ

    assigned = set(try_role_map1.values())

    # チーム2のレーン割り当て（チーム1で使ったレーンは再利用OK）
    valid_team2 = False
    for team2_roles in permutations(lanes):
        ...
        for uid, lane in zip(team2_ids, team2_roles):
            ...
            if prefs and lane not in prefs and 'fill' not in prefs:
                success2 = False
                break
            try_role_map2[uid] = lane
        if success2:
            valid_team2 = True
            break

    if not valid_team2:
        continue  # チーム2が希望レーンで組めない場合はスキップ

    # 合計値とレーン別の差の検証に進む
    team1 = [(uid, try_role_map1[uid], data[uid][try_role_map1[uid]]) for uid in team1_ids]
    team2 = [(uid, try_role_map2[uid], data[uid][try_role_map2[uid]]) for uid in team2_ids]

    team1_score = sum(member[2] for member in team1)
    team2_score = sum(member[2] for member in team2)
    total_diff = abs(team1_score - team2_score)

    lane_diffs = [
        abs(data[[uid for uid, role, _ in team1 if role == lane][0]][lane] -
            data[[uid for uid, role, _ in team2 if role == lane][0]][lane])
        for lane in lanes
    ]

    if total_diff <= team_diff and all(diff <= lane_diff for diff in lane_diffs):
        best_team1 = team1
        best_team2 = team2
        best_score_diff = total_diff
        best_lane_diffs = lane_diffs
        break  # 条件を満たす最初のペアを採用
